...
// with `any`, which cleared the error here and was wrong everywhere else: a self-referential interface
// reaches the same window with no getter and no inference anywhere in the program, and an unpatched
// compiler answers it from the index signature, so `any` silently accepted an indexed access every
// other compiler rejects. Completing the lookup is what separates the two -- a name no base carries is
// still absent.
...
// The recursive one resolves too, three levels in.
